...
app.intent('StartIntent'),
    ...
    function (request, response) {
        ...
        response.say("How about we start with what you like. Give me an idea of a show or movie you really enjoyed.");
...
function filter_movie(response, title){
    var filterlist = retrieveWatchingRecs();
    var winninglist = [];
    var genres = [];
...
// likes == attributes about a show or movie (As array). Shows is an array of shows

  var filtered = [];
  for (var i = 0; i < shows.length; i++) {
    shows[i].numberOfMatchingAttributes = 0;
    for (var j = 0; j < shows[i].likes.length; j++) {
      for(var k = 0; k < attributes.length; k++){
        if(shows[i].likes[j].name.toLowerCase() == attributes[k].name.toLowerCase()) {
